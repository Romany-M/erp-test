...
export default function AdvancesPage() {
  const { workers, advances, addAdvance, deleteAdvance } = useApp();
  const [showAdd, setShowAdd] = useState(false);
  const [search, setSearch] = useState('');
  const [dateFrom, setDateFrom] = useState(() => todayISO().substring(0, 7) + '-01');
  const [dateTo, setDateTo] = useState('');
  const [form, setForm] = useState({
    date: todayISO(), workerId: '', amount: '', notes: '', paymentType: 'cash',
  });
  ...
  const monthWeeks = useMemo(() => {
    ...
    return weeks.map(w => {
      const weekAdvances = advances.filter(a => a.date >= w.start && a.date <= w.end);
      const total = weekAdvances.reduce((s, a) => s + (a.amount || 0), 0);
      let status = 'upcoming';
  ...
  const filtered = useMemo(() => {
    let data = [...advances].sort((a, b) => new Date(b.date) - new Date(a.date));
    if (search) {
      data = data.filter(a => {
        const w = workers.find(wk => wk.id === a.workerId);
        return w?.name.includes(search) || w?.code.includes(search);
      });
    }
    if (dateFrom) data = data.filter(a => a.date >= dateFrom);
    if (dateTo) data = data.filter(a => a.date <= dateTo);
    return data;
  }, [advances, workers, search, dateFrom, dateTo]);

  const handleSubmit = (e) => {
    e.preventDefault();
    if (!form.workerId || !form.amount) return;
    addAdvance(form);
    setForm({ date: todayISO(), workerId: '', amount: '', notes: '', paymentType: 'cash' });
    setShowAdd(false);
  };

  const getPaymentLabel = (type) => PAYMENT_TYPES.find(p => p.value === type)?.label || type;

  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <h2 className="text-2xl font-bold text-gray-800">السلف</h2>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
        <div className="flex items-center justify-between mb-3">
          <h3 className="font-bold text-gray-800">إجمالي السلف حسب الأسابيع</h3>
        ...
        <div className="divide-y divide-gray-100">
          {monthWeeks.map(w => (
            <div key={w.key} className="flex items-center justify-between py-2 text-sm">
              ...
              <span className="font-bold text-orange-600">{(w.total || 0).toLocaleString('ar-EG')} ج.م</span>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.map(a => {
                ...
                return (
                  <tr key={a.id} className="hover:bg-gray-50 transition">
                    <td className="px-4 py-3">{new Date(a.date).toLocaleDateString('ar-EG')}</td>
                    <td className="px-4 py-3 font-medium">{w?.name || 'محذوف'}</td>
                    <td className="px-4 py-3 font-bold text-orange-600">{(a.amount || 0).toLocaleString('ar-EG')} ج.م</td>
                    <td className="px-4 py-3">
                      <span className={`px-2 py-1 rounded text-xs font-medium ${
                        a.paymentType === 'cash' ? 'bg-green-100 text-green-700' :
                        a.paymentType === 'insta' ? 'bg-purple-100 text-purple-700' :
                        'bg-blue-100 text-blue-700'
                      }`}>{getPaymentLabel(a.paymentType)}</span>
                    </td>
                    <td className="px-4 py-3 text-gray-500">{a.notes || '-'}</td>
                    <td className="px-4 py-3">
                      <button onClick={() => { if (confirm('هل تريد حذف هذه السلفة؟')) deleteAdvance(a.id); }}
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={6} className="text-center py-8 text-gray-400">لا توجد سلف مسجلة</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {showAdd && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={() => setShowAdd(false)}>
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-bold text-gray-800 mb-4">إضافة سلفة جديدة</h3>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex gap-3 justify-end">
                ...
                <button type="submit"
                  className="px-6 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-lg transition font-medium">إضافة</button>
              </div>
            </form>
          </div>
        </div>
