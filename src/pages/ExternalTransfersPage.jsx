...
export default function ExternalTransfersPage() {
  const { externalTransfers, addExternalTransfer, deleteExternalTransfer } = useApp();
  const [showAdd, setShowAdd] = useState(false);
  const [search, setSearch] = useState('');
  const [dateFrom, setDateFrom] = useState(() => todayISO().substring(0, 7) + '-01');
  const [dateTo, setDateTo] = useState('');
  const [form, setForm] = useState({
    date: todayISO(), recipient: '', phone: '', amount: '', transactionId: '', walletName: '', notes: '', paymentType: 'cash',
  });
  ...
  const monthWeeks = useMemo(() => {
    ...
    return weeks.map(w => {
      const weekTransfers = externalTransfers.filter(t => t.date >= w.start && t.date <= w.end);
  ...
  }, [dateFrom, externalTransfers]);

  const filtered = useMemo(() => {
    let data = [...externalTransfers].sort((a, b) => new Date(b.date) - new Date(a.date));
    if (search) {
      data = data.filter(t =>
        t.recipient?.includes(search) || t.phone?.includes(search) || t.transactionId?.includes(search)
      );
    }
    if (dateFrom) data = data.filter(t => t.date >= dateFrom);
    if (dateTo) data = data.filter(t => t.date <= dateTo);
    return data;
  }, [externalTransfers, search, dateFrom, dateTo]);

  const handleSubmit = (e) => {
    e.preventDefault();
    if (!form.recipient || !form.amount) return;
    addExternalTransfer(form);
    setForm({ date: todayISO(), recipient: '', phone: '', amount: '', transactionId: '', walletName: '', notes: '', paymentType: 'cash' });
    setShowAdd(false);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-800">تحويلات خارجية</h2>
          <p className="text-xs text-gray-500 mt-0.5">تحويلات لجهات خارجية غير العمال — بند مستقل لا يُحتسب ضمن رواتب العمال</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
        <div className="flex flex-wrap gap-3">
          <input type="text" placeholder="بحث بالمستلم أو رقم الهاتف أو رقم المعاملة..." value={search} onChange={e => setSearch(e.target.value)}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
        <div className="flex items-center justify-between mb-3">
          <h3 className="font-bold text-gray-800">إجمالي التحويلات الخارجية حسب الأسابيع</h3>
        ...
        <div className="divide-y divide-gray-100">
          {monthWeeks.map(w => (
            <div key={w.key} className="flex items-center justify-between py-2 text-sm">
              ...
              <span className="font-bold text-cyan-600">{(w.total || 0).toLocaleString('ar-EG')} ج.م</span>
            </div>
          ))}
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="px-4 py-3 text-right font-semibold text-gray-600">المستلم</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.map(t => (
                <tr key={t.id} className="hover:bg-gray-50 transition">
                  ...
                  <td className="px-4 py-3 font-medium">{t.recipient || '—'}</td>
                  ...
                  <td className="px-4 py-3 text-gray-500" dir="ltr">{t.transactionId || '-'}</td>
                  <td className="px-4 py-3 text-gray-500">{t.notes || '-'}</td>
                  <td className="px-4 py-3">
                    <button onClick={() => { if (confirm('هل تريد حذف هذه التحويلة الخارجية؟')) deleteExternalTransfer(t.id); }}
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={9} className="text-center py-8 text-gray-400">لا توجد تحويلات خارجية مسجلة</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {showAdd && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={() => setShowAdd(false)}>
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-bold text-gray-800 mb-4">إضافة تحويلة خارجية</h3>
