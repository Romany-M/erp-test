...
export default function CustodyPage() {
  const { workers, custody, addCustody, deleteCustody } = useApp();
  ...
  const filtered = useMemo(() => {
    let data = [...custody].sort((a, b) => new Date(b.date) - new Date(a.date));
    if (search) {
      data = data.filter(c => {
    ...
    if (filterRole) {
      data = data.filter(c => {
        const w = workers.find(wk => wk.id === c.workerId);
        return w?.role === filterRole;
      });
    }
    if (dateFilter) data = data.filter(c => c.date === dateFilter);
    return data;
  }, [custody, workers, search, dateFilter, filterRole]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <h2 className="text-2xl font-bold text-gray-800">العهد</h2>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.map(c => {
                ...
                return (
                  <tr key={c.id} className="hover:bg-gray-50 transition">
                    <td className="px-4 py-3">{new Date(c.date).toLocaleDateString('ar-EG')}</td>
                    <td className="px-4 py-3 font-medium">{w?.name || 'محذوف'}</td>
                    <td className="px-4 py-3 font-bold text-blue-600">{(c.amount || 0).toLocaleString('ar-EG')} ج.م</td>
                    <td className="px-4 py-3">
                      <span className={`px-2 py-1 rounded text-xs font-medium ${
                        c.paymentType === 'cash' ? 'bg-green-100 text-green-700' :
                        c.paymentType === 'insta' ? 'bg-purple-100 text-purple-700' :
                        'bg-blue-100 text-blue-700'
                      }`}>{PAYMENT_TYPES.find(p => p.value === c.paymentType)?.label || c.paymentType}</span>
                    </td>
                    <td className="px-4 py-3 text-gray-500">{c.notes || '-'}</td>
                    <td className="px-4 py-3">
                      <button onClick={() => { if (confirm('هل تريد حذف هذه العهدة؟')) deleteCustody(c.id); }}
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={6} className="text-center py-8 text-gray-400">لا توجد عهد مسجلة</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {showAdd && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={() => setShowAdd(false)}>
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-bold text-gray-800 mb-4">إضافة عهدة جديدة</h3>
